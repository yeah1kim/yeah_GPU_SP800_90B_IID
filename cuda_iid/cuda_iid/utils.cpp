...
INPUT_SAMPLE_SIZE:
	...
	printf("   If all 0, set as the defaults(2500, 10, 250), and at least 2.5GB of GPU global memory is used. \n");
	...
	if (UINT64_MAX == s) {
		...
		for (k = 0; k < 3; k++) {
			for (i = 0; i < 2; i++) {
				for (j = 0; j < 2; j++) {
					if (i + j == k) {
						UV = (unsigned __int64)BigInt1[i] * (unsigned __int64)BigInt2[j];
